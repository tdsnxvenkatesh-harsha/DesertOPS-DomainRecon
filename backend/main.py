...
async def analyze(request: AnalyzeRequest):
    ...
    has_email = detect_email_functionality(soup, text)
    email_functionality = "Yes" if has_email else "No"
    ...
    llm_result = await call_llm(domain, text[:3000], signals)
    if llm_result:
        email_functionality = llm_result.get("email_functionality", email_functionality)
    ...
    release_recommendation = detect_release_recommendation(
        content_status, likely_purpose, has_email, last_update
    )
